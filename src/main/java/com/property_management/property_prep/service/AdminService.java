...
import com.property_managemepackage com.property_management.property_prep.service;
...
public class AdminService {
    ...
    // 2. Get all Managers with property counts - OPTIMIZED (No N+1)
    public List<ManagerSummaryDTO> getAllManagers() {
        // 🔥 ONE SINGLE QUERY to get all managers + their property counts
        // Uses a custom @Query in PropertyRepository to fetch counts in one go
        List<Object[]> results = propertyRepository.countPropertiesByManager();

        // Convert results to a Map<ManagerId, Count>
        Map<Long, Long> propertyCountMap = results.stream()
                .collect(Collectors.toMap(
                        row -> ((User) row[0]).getId(),   // Key: Manager ID
                        row -> (Long) row[1]              // Value: Property Count
                ));

        // Fetch all managers in one query
        List<User> managers = userRepository.findByRole(RoleType.LANDLORD);

        // Build DTOs using the pre‑fetched counts (No N+1 queries!)
        return managers.stream()
                .map(m -> new ManagerSummaryDTO(
                        m.getId(),
                        m.getUsername(),
                        m.getEmail(),
                        m.getSubscription().name(),
                        propertyCountMap.getOrDefault(m.getId(), 0L)
                ))
    ...
    // 4. Update a manager's subscription
...
}nt.property_prep.dto.ManagerSummaryDTO;
import com.property_management.property_prep.dto.UserProfileDTO;
import com.property_management.property_prep.entity.User;
import com.property_management.property_prep.enums.RoleType;
import com.property_management.property_prep.repository.PropertyRepository;
